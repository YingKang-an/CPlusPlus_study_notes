...
/****************************************************************
* 编译器对 类外重载 运算符的隐式翻译过程（核心对比点）
* 核心规则：全局函数版运算符重载，无隐式参数，两个操作数都必须显式传递
* 1. 表达式：a + AA  → 编译器翻译为：operator+(a, AA)
*    - 左操作数 a：作为全局函数 operator+ 的第一个显式参数
*    - 右操作数 AA：作为全局函数 operator+ 的第二个显式参数
*    - 功能：调用全局函数 operator+(const int a[], const Arr& obj)，完成数组 + 对象
* 
* 【类内 vs 类外 翻译规则对比】
* 类内重载（对象 + 数组）：AA + c → AA.operator+(c) （左操作数是 this）
* 类外重载（数组 + 对象）：a + AA → operator+(a, AA) （两个操作数都显式传参）
* 
* 【关键结论】
* - 类内重载：左操作数必须是 类的对象（因为依赖 this 指针）
* - 类外重载：无操作数类型限制（只要符合参数列表），但需要友元访问私有成员
****************************************************************/